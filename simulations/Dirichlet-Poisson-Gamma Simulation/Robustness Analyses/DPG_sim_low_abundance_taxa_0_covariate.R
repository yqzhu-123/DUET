...
# Modified: fixed 50 low-abundance causal OTUs, no confounder, no prop.diff grid
...
## 1) Causal OTUs — fixed 50 low-abundance taxa
causal.otus.idx    <- sample(which(pi_est >= 0.0005 & pi_est <= 0.001), 50)
noncausal.otus.idx <- setdiff(1:n.otus, causal.otus.idx)
causal.otus        <- taxa_names[causal.otus.idx]
noncausal.otus     <- setdiff(taxa_names, causal.otus)

## 2) Structural zero OTUs — fixed once
##    5 from causal + 5 from non-causal = 10 total
sz_causal    <- sample(causal.otus.idx, n_szero_causal)
sz_noncausal <- sample(noncausal.otus.idx, n_szero_noncausal)
szero.idx    <- c(sz_causal, sz_noncausal)

## 3) Bias factors — fixed once
set.seed(1)
bf1 <- rep(1, n.otus)
bf2 <- rep(1, n.otus)
if (have_bias) {
...
simulate_one_param <- function(c_val, beta_val, nsam_val, fdr_target, phi, seed) {
...
combos0$label <- sprintf("c=%g|nsam=%d_%d|fdr=%.2f|phi=%g",
                         c_grid[combos0$ic],
