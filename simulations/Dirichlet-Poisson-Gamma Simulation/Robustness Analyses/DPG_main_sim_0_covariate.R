...
beta_grid           <- c(5, 6, 7, 8, 9, 10)
prop.diff_grid      <- c(0.05)
nsam_grid           <- list(c(50, 50))
fdr_grid            <- c(0.2)
phi_grid            <- c(0.75, 1, 1.25)   # per-OTU overdispersion (shared across all OTUs)


##########################################################
## Fixed components — outside Monte Carlo loop
## (causal OTUs, structural zero OTUs, bias: fixed once)
##########################################################
...
## 3) Bias factors — fixed once per prop.diff (dedicated seed)
...
## (causal, szero, bias accessed from global scope — fixed)
## Count generation: Dirichlet-Poisson-Gamma (phi as scenario parameter)
##########################################################

simulate_one_param <- function(c_val, beta_val, nsam_val, fdr_target, ip, phi, seed) {
  ...
  ## 1) Depth (random per replicate, rnorm)
  ...
  ## 3) Dirichlet sampling (sample-level variation)
  alpha <- c_val * pi_est
  ...
  ## 4) Causal effect (deterministic, using fixed causal.otus.idx)
  ...
  ## 5) Bias (deterministic, using fixed bias.factor)
  if (have_bias == 1) {
    pi.table1.sim <- sweep(pi.table1.sim, 2, bias.factor1, "*")
    pi.table2.sim <- sweep(pi.table2.sim, 2, bias.factor2, "*")
  }

  ## 5b) Structural zeros — applied after bias, before normalization
  ...
  ## 6) Normalize proportions (renormalization absorbs the zeroed-out mass)
  pi.table1.sim <- pi.table1.sim / rowSums(pi.table1.sim)
  pi.table2.sim <- pi.table2.sim / rowSums(pi.table2.sim)

  ## 7) Poisson-Gamma count generation (mu = depth * proportion, phi from scenario grid)
  ...
  ## 8) Filter & pool (no structural zero filtering)
  ...
  count_SK <- rbind(cbind(M = M1, W1), cbind(M = M2, W2))

  data_x <- NULL
  W      <- as.data.frame(count_SK[, -1])
  M      <- count_SK[, 1]
  ...
  ## DUET
  res.DUET <- DUET(
    W = W, class_K = class_K, data_x = NULL, M = M, y = y, T_var = T_var,
  ...
  res.locom1 <- locom(otu.table = otu.table1.sim.filter, Y = Y, C = NULL,
                      n.perm.max = 50000, fdr.nominal = fdr_target,
                      n.cores = n_cores, n.rej.stop = n_rej_stop)

  res.locom2 <- locom(otu.table = otu.table2.sim.filter, Y = Y, C = NULL,
                      n.perm.max = 50000, fdr.nominal = fdr_target,
                      n.cores = n_cores, n.rej.stop = n_rej_stop)

  res.locom.pool <- locom(otu.table = otu.table.sim.pool.filter, Y = Y, C = NULL,
                          n.perm.max = 50000, fdr.nominal = fdr_target,
                          n.cores = n_cores, n.rej.stop = n_rej_stop)

  ## Summarize Com2seq results
  ## causal.otus.in.pool: only causal OTUs that survived pooled filter (matches DUET's T_var denominator)
...
combos0$label <- sprintf("c=%g|prop.diff=%.2f|nsam=%d_%d|fdr=%.2f|phi=%g",
                         c_grid[combos0$ic],
...
## k0 already incorporates iphi (via expand.grid), so distinct phi scenarios get distinct seeds
