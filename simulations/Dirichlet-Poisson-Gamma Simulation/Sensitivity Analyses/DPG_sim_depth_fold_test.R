...
simulate_one_param <- function(c_val, beta_val, nsam_val, fdr_target, ip, phi, depth_fold, seed) {
...
combos0$label <- sprintf("c=%g|prop.diff=%.2f|nsam=%d_%d|fdr=%.2f|phi=%g|depth_fold=%g",
                         c_grid[combos0$ic],
...
## k0 already incorporates iphi and idf (via expand.grid), so distinct phi x depth_fold
## scenarios get distinct seeds
