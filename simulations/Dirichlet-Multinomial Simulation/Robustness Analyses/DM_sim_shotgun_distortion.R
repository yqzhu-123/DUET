...
# table (pi.table2.sim), inserted AFTER bias and BEFORE normalization.
# distort.prop is a new grid parameter analogous to drop.prop.
...
simulate_one_param <- function(c_val, beta_val, nsam_val, fdr_target, disp_val,
                               distort_val, ip, seed) {
...
## combos without beta (but includes disp and distort)
combos0 <- expand.grid(
  ic = seq_along(c_grid),
...
combos0$label <- sprintf("c=%g|prop.diff=%.2f|nsam=%d_%d|fdr=%.2f|disp=%.3f|distort=%.3f",
                         c_grid[combos0$ic],
