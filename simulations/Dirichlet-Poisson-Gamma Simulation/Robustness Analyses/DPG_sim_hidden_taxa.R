...
# Hidden taxa (drop.prop) robustness check
# Based on All-DPG-phi-1c-10sz.R
# Dirichlet-Poisson-Gamma (DPG) count generation
# Modified: randomly drop a proportion of taxa after perturbation,
#           before normalization, inside each Monte Carlo replicate
# phi as scenario grid parameter (shared across all OTUs)
# Structural zeros: 5 from causal + 5 from non-causal, fixed outside MC loop
#                   group assignment random per replicate, no filtering
...
beta_grid           <- c(3, 4, 5, 6, 7, 8)
prop.diff_grid      <- c(0.05)
nsam_grid           <- list(c(50, 50))
fdr_grid            <- c(0.2)
phi_grid            <- c(1.25)   # per-OTU overdispersion (shared across all OTUs)
drop.prop_grid      <- c(0.1, 0.2, 0.3)              # hidden taxa proportions


##########################################################
## Fixed components — outside Monte Carlo loop
## (causal OTUs, structural zero OTUs, confounders, bias: fixed once)
##########################################################
...
## 2) Structural zero OTUs — fixed once per prop.diff
##    5 from causal + 5 from non-causal = 10 total
##    szero.idx.list[[ip]]: integer index vector into 1:n.otus
szero.idx.list <- vector("list", length(prop.diff_grid))
for (ip in seq_along(prop.diff_grid)) {
  sz_causal    <- sample(causal.otus.idx.list[[ip]], n_szero_causal)
  sz_noncausal <- sample(noncausal.otus.idx.list[[ip]], n_szero_noncausal)
  szero.idx.list[[ip]] <- c(sz_causal, sz_noncausal)
}

## 3) Confounding OTUs — fixed once
...
## 4) Bias factors — fixed once per prop.diff (dedicated seed)
...
## (causal, szero, confounding, bias accessed from global scope — fixed)
## Count generation: Dirichlet-Poisson-Gamma (phi as scenario parameter)
## NEW: drop.prop — proportion of taxa randomly hidden per replicate
##########################################################

simulate_one_param <- function(c_val, beta_val, nsam_val, fdr_target, ip, phi, drop.prop, seed) {
  ...
  ## 7b) Structural zeros — applied after bias, before hidden taxa drop
  ##     Each szero taxon is randomly assigned to be zero in group1 OR group2.
  if (length(szero.idx) > 0) {
    szero.flag   <- sample(c(1L, 2L), length(szero.idx), replace = TRUE)
    grp1.samples <- which(Y == 0)   # group1 sample rows
    grp2.samples <- which(Y == 1)   # group2 sample rows
    sz.grp1 <- szero.idx[szero.flag == 1]   # zero in group1
    sz.grp2 <- szero.idx[szero.flag == 2]   # zero in group2
    if (length(sz.grp1) > 0) {
      pi.table1.sim[grp1.samples, sz.grp1] <- 0   # platform 1
      pi.table2.sim[grp1.samples, sz.grp1] <- 0   # platform 2
    }
    if (length(sz.grp2) > 0) {
      pi.table1.sim[grp2.samples, sz.grp2] <- 0   # platform 1
      pi.table2.sim[grp2.samples, sz.grp2] <- 0   # platform 2
    }
  }

  ## 8) Hidden taxa — randomly drop columns BEFORE normalization
  ##    Simulates incomplete reference database: some taxa exist in the
  ##    ecosystem but are not in the reference, so they cannot be detected.
  ##    Dropped taxa are removed from pi tables; their probability mass is
  ##    absorbed by remaining taxa during normalization.
  n_drop <- round(n.otus * drop.prop)
  if (n_drop > 0) {
    drop_idx  <- sample(n.otus, n_drop)
    keep_idx  <- setdiff(seq_len(n.otus), drop_idx)

    hidden_taxa <- taxa_names[drop_idx]

    pi.table1.sim <- pi.table1.sim[, keep_idx, drop = FALSE]
    pi.table2.sim <- pi.table2.sim[, keep_idx, drop = FALSE]

    ## Update causal/noncausal for this replicate (exclude hidden taxa)
    causal.otus    <- setdiff(causal.otus, hidden_taxa)
    noncausal.otus <- setdiff(noncausal.otus, hidden_taxa)

    ## Subset phi vector to match remaining OTUs
    phi <- phi[keep_idx]
  } else {
    keep_idx <- seq_len(n.otus)
  }

  n_otus_eff       <- ncol(pi.table1.sim)
  taxa_names_local <- colnames(pi.table1.sim)

  ## 9) Normalize proportions (renormalization absorbs the zeroed-out / dropped mass)
  pi.table1.sim <- pi.table1.sim / rowSums(pi.table1.sim)
  pi.table2.sim <- pi.table2.sim / rowSums(pi.table2.sim)

  ## 10) Poisson-Gamma count generation (mu = depth * proportion, phi from scenario grid)
  mu1 <- pi.table1.sim * depth1.sim   # n_sam x n_otus_eff (R recycles depth vector row-wise)
  mu2 <- pi.table2.sim * depth2.sim

  phi_safe <- pmax(phi, 1e-8)

  otu.table1.sim <- matrix(0L, n_sam, n_otus_eff)
  otu.table2.sim <- matrix(0L, n_sam, n_otus_eff)

  idx   <- (phi >= 1e-8)   # overdispersed OTUs: use Gamma-Poisson
  idx_p <- !idx             # Poisson OTUs
  ...
  if (any(idx_p)) {
    ...
    otu.table1.sim[, idx_p] <- matrix(rpois(n_sam * sum(idx_p), lambda = c(mu1_p)), nrow = n_sam)

    mu2_p <- mu2[, idx_p, drop = FALSE]
    otu.table2.sim[, idx_p] <- matrix(rpois(n_sam * sum(idx_p), lambda = c(mu2_p)), nrow = n_sam)
  }

  colnames(otu.table1.sim) <- colnames(otu.table2.sim) <- taxa_names_local
  rn <- sprintf("id_%03d", seq_len(nrow(otu.table1.sim)))
  rownames(otu.table1.sim) <- rn
  rownames(otu.table2.sim) <- rn

  ## 11) Filter & pool (no structural zero filtering)
  ...
  ## Summarize results
  ## causal.otus.in.pool: only visible causal OTUs that survived pooled filter
  causal.otus.in.pool <- causal.otus[causal.otus %in% colnames(W1)]
  ...
  ## summarize results
  ## NOTE: causal.otus / causal.otus.in.pool already exclude hidden taxa,
  ##       so Power denominator = only visible causal taxa (fair comparison).
  summarize_otu_results <- function(qvalue, causal.otus.ref, fdr.target=fdr_target) {
    otu.detected <- (if (!is.null(colnames(qvalue))) colnames(qvalue) else names(qvalue))[which(qvalue < fdr.target)]
    n.otu <- length(otu.detected)
    if (n.otu > 0) {
      sen <- sum(otu.detected %in% causal.otus.ref) / max(length(causal.otus.ref), 1)
      fdr <- (n.otu - sum(otu.detected %in% causal.otus.ref)) / n.otu
    } else {
      sen <- 0
      fdr <- 0
    }
  ...
  otu.new.omni <- summarize_otu_results(res.Com2seq$q.taxa.omni, causal.otus.in.pool)
  otu.comp.locom <- summarize_otu_results(q.comp, causal.otus.in.pool)
  otu.pool.locom <- summarize_otu_results(res.locom.pool$q.otu, causal.otus.in.pool)
  otu.locom.1 <- summarize_otu_results(res.locom1$q.otu, causal.otus.in.pool)
  otu.locom.2 <- summarize_otu_results(res.locom2$q.otu, causal.otus.in.pool)
...
Kf    <- length(fdr_grid)
Kb    <- length(beta_grid)
Kphi  <- length(phi_grid)
Kdrop <- length(drop.prop_grid)
...
combos0 <- expand.grid(
  ...
  iphi   = seq_along(phi_grid),
  idrop  = seq_along(drop.prop_grid),
  KEEP.OUT.ATTRS = FALSE
)
K0 <- nrow(combos0)
combos0$k0 <- seq_len(K0)
combos0$label <- sprintf("c=%g|prop.diff=%.2f|nsam=%d_%d|fdr=%.2f|phi=%g|drop=%.2f",
                         c_grid[combos0$ic],
                         prop.diff_grid[combos0$ip],
                         sapply(nsam_grid[combos0$i_nsam], `[`, 1),
                         sapply(nsam_grid[combos0$i_nsam], `[`, 2),
                         fdr_grid[combos0$ifdr],
                         phi_grid[combos0$iphi],
                         drop.prop_grid[combos0$idrop])
...
## cores
n_cores_outer <- min(94, parallel::detectCores() - 1L)
...
work_fun_crn <- function(i) {
  ...
  phi        <- rep(phi_grid[iphi], n.otus)   # length-n.otus vector, all same value
  drop.prop  <- drop.prop_grid[idrop]
  ...
  for (ib in seq_along(beta_grid)) {
    for (m in methods) {
      ...
      summary_rows[[row_i]] <- data.frame(
        ...
        phi             = phi_grid[combos0$iphi[k0]],
        drop.prop       = drop.prop_grid[combos0$idrop[k0]],
