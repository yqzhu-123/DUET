...
simulate_one_param <- function(c_val, beta_val, nsam_val, fdr_target, disp_val, ip, drop.prop, seed) {
  ...
  ## 10) Count data (Dirichlet-Multinomial or Multinomial)
  otu.table1.sim <- matrix(0, n_sam, n_otus_eff)
  otu.table2.sim <- matrix(0, n_sam, n_otus_eff)
  ...
  ## summarize results
  ## NOTE: causal.otus here has already been updated to exclude hidden taxa,
  ...
  summarize_otu_results <- function(qvalue, causal.otus, noncausal.otus, fdr.target=fdr_target) {
    ...
    if (n.otu > 0) {
      sen <- sum(otu.detected %in% causal.otus) / max(length(causal.otus), 1)
...
## combos without beta (but includes disp and drop.prop)
combos0 <- expand.grid(
  ic = seq_along(c_grid),
...
combos0$label <- sprintf("c=%g|prop.diff=%.2f|nsam=%d_%d|fdr=%.2f|disp=%.3f|drop=%.2f",
                         c_grid[combos0$ic],
  ...
  out_beta <- lapply(seq_along(beta_grid), function(ib){
    ...
    vals <- try(
      simulate_one_param(
        ...
        ip=ip, drop.prop=drop.prop, seed=seed_r
      ),
      silent = TRUE
    )
