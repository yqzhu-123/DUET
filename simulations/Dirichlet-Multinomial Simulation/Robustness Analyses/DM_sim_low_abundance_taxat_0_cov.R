...
# Modified: fixed 50 low-abundance causal OTUs, no confounder
...
simulate_one_param <- function(c_val, beta_val, nsam_val, fdr_target, disp_val, seed) {
...
## combos without beta (but includes disp)
combos0 <- expand.grid(
  ic = seq_along(c_grid),
...
combos0$label <- sprintf("c=%g|nsam=%d_%d|fdr=%.2f|disp=%.3f",
                         c_grid[combos0$ic],
