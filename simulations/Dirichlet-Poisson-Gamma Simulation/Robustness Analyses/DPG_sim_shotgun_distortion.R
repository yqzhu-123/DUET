...
# Modified: causal OTUs, bias fixed outside Monte Carlo loop
...
#
# === Distortion robustness variant ===
# Adds per-replicate, per-taxon multiplicative distortion ONLY to the shotgun
# table (pi.table2.sim), inserted AFTER bias + structural zeros and BEFORE
# normalization. distort.prop is a new grid parameter analogous to drop.prop.
...
distort_grid        <- c(0.05, 0.1, 0.2)            # NEW: fraction of taxa distorted in shotgun
...
simulate_one_param <- function(c_val, beta_val, nsam_val, fdr_target,
                               distort_val, ip, phi, seed) {
  ...
  ## 7b) Structural zeros — applied after bias, before normalization
  ##     Each szero taxon is randomly assigned to be zero in group1 OR group2.
  ##     Applied to BOTH platform tables: structural zero means the taxon is
  ##     absent from that group in the ecosystem, regardless of platform.
  ##     No filtering: methods must handle structural zeros directly.
  ...
  ## 7.5) Shotgun-side distortion (ONLY pi.table2.sim, before normalize)
  ##      Randomly pick distort_val * n.otus taxa (from ALL taxa, not distinguishing
  ##      causal / non-causal), multiply their shotgun proportions by a per-taxon
  ##      factor drawn from Uniform(0.5, 1.5). This mimics shotgun-only taxon
  ##      mismeasurement (assembly/mapping errors, database incompleteness, etc.).
  if (distort_val > 0) {
    n_distort <- max(1, round(n.otus * distort_val))
    set.seed(seed + 600000L)
    distort.idx <- sample(1:n.otus, n_distort)
    distort.factors <- runif(n_distort, 0.5, 1.5)
    pi.table2.sim[, distort.idx] <- sweep(
      pi.table2.sim[, distort.idx, drop = FALSE],
      2, distort.factors, "*"
    )
  }

  ## 8) Normalize proportions (renormalization absorbs the zeroed-out mass)
  pi.table1.sim <- pi.table1.sim / rowSums(pi.table1.sim)
  pi.table2.sim <- pi.table2.sim / rowSums(pi.table2.sim)

  ## 9) Poisson-Gamma count generation (mu = depth * proportion, phi from scenario grid)
  mu1 <- pi.table1.sim * depth1.sim   # n_sam x n.otus (R recycles depth vector row-wise)
  mu2 <- pi.table2.sim * depth2.sim

  phi_safe <- pmax(phi, 1e-8)

  otu.table1.sim <- matrix(0L, n_sam, n.otus)
  otu.table2.sim <- matrix(0L, n_sam, n.otus)
  ...
  ## 10) Filter & pool (no structural zero filtering)
  ...
  ## Summarize results
  ## causal.otus.in.pool: only causal OTUs that survived pooled filter
...
combos0 <- expand.grid(
  ...
  idistort = seq_along(distort_grid),
  KEEP.OUT.ATTRS = FALSE
)
K0 <- nrow(combos0)
combos0$k0 <- seq_len(K0)
combos0$label <- sprintf("c=%g|prop.diff=%.2f|nsam=%d_%d|fdr=%.2f|phi=%g|distort=%.3f",
                         c_grid[combos0$ic],
...
## k0 already incorporates iphi and idistort (via expand.grid)
...
## cores
n_cores_outer <- min(47, parallel::detectCores() - 1L)
...
work_fun_crn <- function(i) {
  ...
  distort_val <- distort_grid[idistort]
  ...
  for (ib in seq_along(beta_grid)) {
    for (m in methods) {
      ...
      summary_rows[[row_i]] <- data.frame(
        ...
        distort.prop    = distort_grid[combos0$idistort[k0]],
